(() => {
  ...
  function playerName() {
    const field = $(".identity-row input");
  ...
  function pageText() {
    const parts = [];
    $$(".comparison-result, .challenge-result, .result-card, .share-studio").forEach((node) => parts.push(node.textContent || ""));
    $$(".share-fields input, .share-studio input, .share-studio textarea").forEach((field) => {
  ...
  function sameSafeLabels(context) {
    const rawMe = (context.myName || "我").trim() || "我";
    const rawTa = (context.opponent || "TA").trim() || "TA";
    return rawMe === rawTa ? { me: "我", ta: "TA" } : { me: rawMe, ta: rawTa };
  }
  ...
  function readContext() {
    ...
    try { sessionStorage.setItem("pack-clean-pk-context", JSON.stringify(activeContext)); } catch {}
    return activeContext;
  }

  function copyText(context) {
    const { me, ta } = sameSafeLabels(context);
    const delta = norm(context.delta);
    const round = `第 ${context.round}/${TOTAL} 关`;
    if (context.state === "faster") return {
      title: `${me} 比 ${ta} 快 ${delta} 秒`,
      text: `${me} PK ${ta}，在《${GAME}》${round}快了 ${delta} 秒！${me} ${norm(context.myElapsed)} 秒，${ta} ${norm(context.targetElapsed)} 秒。轮到你追我。`,
    };
    if (context.state === "slower") return {
      title: `${me} 比 ${ta} 慢 ${delta} 秒`,
      text: `${me} PK ${ta}，在《${GAME}》${round}慢了 ${delta} 秒。${me} ${norm(context.myElapsed)} 秒，${ta} ${norm(context.targetElapsed)} 秒。下一局追回来。`,
    };
    return {
      title: `${me} 和 ${ta} 打平了`,
      text: `${me} PK ${ta}，在《${GAME}》${round}用时打平！都是 ${norm(context.myElapsed)} 秒。再来一局分胜负。`,
    };
  }
  ...
  function fit(ctx, text, x, y, maxWidth, size, color = "#fff", weight = 1000) {
  ...
  function item(ctx, x, y, color, icon, rotate = 0) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotate);
  ...
  function drawPoster(canvas, context) {
    ...
    ctx.fillStyle = "#ed3048"; ctx.fillRect(0, 0, W / 2, H);
    ctx.fillStyle = "#2451d7"; ctx.fillRect(W / 2, 0, W / 2, H);
    ctx.fillStyle = "#10121b"; ctx.fillRect(0, 0, W, 54);
    ...
    ctx.fillStyle = "#ffe14a";
    ctx.beginPath(); ctx.moveTo(492, 54); ctx.lineTo(628, 54); ctx.lineTo(558, 510); ctx.lineTo(656, 510); ctx.lineTo(452, H); ctx.lineTo(515, 760); ctx.lineTo(420, 760); ctx.closePath(); ctx.fill();
    ctx.globalAlpha = .16;
    ctx.fillStyle = "#fff";
    ...
    fit(ctx, "PK 回击战", 540, 122, 820, 56, "#fff36b");
    fit(ctx, `${GAME} · 第 ${context.round}/${TOTAL} 关`, 540, 184, 860, 32, "#f0fbff", 900);
    fillRound(ctx, 58, 260, 430, 380, 40, "#e92740");
    fillRound(ctx, 592, 260, 430, 380, 40, "#1c52e0");
    fit(ctx, me, 273, 326, 330, 56);
    fit(ctx, ta, 807, 326, 330, 56);
    fit(ctx, `${norm(context.myElapsed)} 秒`, 273, 560, 330, 80, faster || tie ? "#fff36b" : "#dce8ff");
    fit(ctx, `${norm(context.targetElapsed)} 秒`, 807, 560, 330, 80, slower || tie ? "#fff36b" : "#dce8ff");
    item(ctx, 155, 430, "#ffb22e", "🧸", -0.14);
    item(ctx, 278, 442, "#ff5b76", "📦", 0.1);
    item(ctx, 382, 425, "#63df76", "🎒", -0.08);
    item(ctx, 695, 430, "#3aa7ff", "📚", 0.12);
    item(ctx, 818, 442, "#a970ff", "👕", -0.1);
    item(ctx, 922, 425, "#26d7e4", "🧃", 0.08);
    ...
    fit(ctx, "VS", 540, 500, 280, 136, "#15122d");

    fillRound(ctx, 100, 894, 880, 176, 48, "rgba(8,8,26,.92)");
    ctx.strokeStyle = "#fff"; ctx.lineWidth = 7; rounded(ctx, 100, 894, 880, 176, 48); ctx.stroke();
    fit(ctx, headline, 540, 944, 790, 52, faster ? "#72ff78" : slower ? "#fff36b" : "#35f2ff");
    fit(ctx, result, 540, 1020, 800, 96);
    fit(ctx, faster ? "我已反超，轮到你追我！" : slower ? "我差一点，下一局追回来！" : "这局打平，再来一箱！", 540, 1150, 900, 48, "#fff6c7");
    fillRound(ctx, 108, 1240, 864, 130, 40, faster ? "#ffe14a" : "#ff6bd1");
    fit(ctx, faster ? "发回去，让 TA 追我" : slower ? "再来一次，追回来" : "再战一局", 540, 1305, 800, 46, "#141226");
    return true;
  }
  ...
  function closeModal() { $("#pack-clean-pk-modal")?.remove(); }

  function openModal(context = readContext()) {
    if (!context) return false;
    activeContext = context;
    closeModal();
    $("#pack-pk-duel-modal")?.remove();
    const oldShare = $(".share-studio");
    ...
    modal.innerHTML = `
      <section class="pack-clean-pk-panel" role="dialog" aria-label="PK 回击战">
        ...
        <div class="pack-clean-pk-head"><span>PK 回击战</span><strong>${text.title}</strong></div>
    ...
    modal.querySelector(".pack-clean-pk-copy").addEventListener("click", () => copy(`${text.title}\n${text.text}\n${location.href}`));
    return true;
  }

  function injectStyle() {
    if ($("#pack-clean-pk-style")) return;
  ...
  function cleanOrdinaryMotto() {
    const motto = $$(".share-fields input").find((input) => !input.closest(".identity-row"));
    if (!motto) return;
    const value = motto.value || "";
    if (/\bPK\b|回击|反超|比.+(?:快|慢)\s*\d+(?:\.\d+)?\s*秒/.test(value) && !readContext()) {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (!context || !/我更快|发回去|打平了|喊TA|晒战绩|分享图片|分享PK图|生成我的装箱战绩|追一次/.test(label)) return;
  ...
  function enforce() {
    ...
    cleanOrdinaryMotto();
    const context = readContext();
    const studio = $(".share-studio");
    if (context && studio && /PK|回击|反超|比.+(?:快|慢)\s*\d+(?:\.\d+)?\s*秒|快了?\s*\d+(?:\.\d+)?\s*秒|慢了?\s*\d+(?:\.\d+)?\s*秒/.test(studio.textContent || pageText())) {
  ...
  window.__packPkControllerV2 = { openModal, readContext, drawPoster, copyText };
})();